...
export class OpenService extends AppServiceBase {
  ...
  addOpen(data): Observable<IResponse<any>> {
    ...
    // const body = `site=${data.site}&enabled=${data.enabled}&jump=${data.jump}&image=${data.image}&sort=${data.sort}&url=${data.url}`;
  ...
  updateOpen(data): Observable<IResponse<any>> {
    ...
    // tslint:disable-next-line:max-line-length
    // const body = `site=${data.site}&jump=${data.jump}&image=${data.image}&sort=${data.sort}&url=${data.url}`;
